...
def main():
    data = pd.read_csv (r'C:\Users\Vnixo\OneDrive\Desktop\userStoryML\storyPointEstimation\Data\TNE_2021-11-11-app.csv')
    ...
    data.drop('issuekey', axis=1, inplace=True)
...
#         cv = cross_val_score(model, X=x_test, y=y_test, cv=StratifiedKFold(shuffle=True, n_splits=5))
...
#     sys.exit()
    ...
    for i in range(100):
        ...
        cv = cross_val_score(rf, X=x_train, y=y_train, cv=StratifiedKFold(shuffle=True, n_splits=5))
        ...
        if auc > best:
           ...
           with open(fr'C:\Users\Vnixo\OneDrive\Desktop\userStoryML\storyPointEstimation\prod_models\model_app1.pickle', 'wb') as f:
               dill.dump(rf, f)
           #dill.dump(transformer, open("tfidf_transformer_owner1.pkl","wb"))
           dill.dump(transformer, open(r"C:\Users\Vnixo\OneDrive\Desktop\userStoryML\storyPointEstimation\transformers\transformer_app1.pickle","wb"))
           break
